...
function resolvePnpm() {
  ...
  for (const name of IS_WIN ? ['npm.cmd', 'npm'] : ['npm']) {
    if (isAvailable(name))
      return {
        file: name,
        prefix: ['exec', '--yes', '--package', `pnpm@${PNPM_VERSION}`, '--', 'pnpm'],
      };
  }
  return null;
}
...
if (!pnpm) {
  fail(
    "Error: none of 'pnpm', 'corepack', or 'npm' was found on PATH. Install Node.js (which provides npm), then retry.",
  );
}
if (pnpm.file.startsWith('npm')) {
  console.log(
    green(
      `pnpm not found; running it on demand via 'npm exec pnpm@${PNPM_VERSION}' (install pnpm for faster launches)`,
    ),
  );
}
...
if (
  run(pnpm.file, [...pnpm.prefix, '--dir', WORKSPACE_ROOT, 'install'], { cwd: WORKSPACE_ROOT }) !==
  0
) {
