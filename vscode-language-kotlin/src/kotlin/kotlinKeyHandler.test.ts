...
import { createDoTest, createParser } from '@jetbrains/vscode-extension-core/testing/keyHandlerTestUtils';
...
describe('Handling key presses in Kotlin files', () => {
    ...
    describe('KDoc handling', () => {
        test(
            'completes parentheses in KDoc comments',
            doTest('/** (<caret> */', '/** (<caret>) */'),
        );
...
fun foo() {
  ...
  println("world")
        ...
        test(
            'starts block comments with leading stars',
            doTest('/*\n<caret>', '/*\n * <caret>\n */'),
        );
